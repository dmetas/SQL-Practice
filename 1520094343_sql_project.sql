...
formatted as a single column. Esure no duplicate data, and order by
the member name. */

SELECT concat(m.surname, ", ", m.firstname) as full_name, f.name as court_name
  FROM Members as m
...
SELECT CONCAT( m.surname,  ", ", m.firstname ) AS full_name, f.name, 
CASE WHEN m.memid =  "0"
THEN f.guestcost
ELSE f.membercost
END AS cost
...
select f.name, sum(CASE WHEN m.memid =  "0"THEN f.guestcost ELSE f.membercost END) as revenue 
from Bookings as b
inner join Facilities as f
  on b.facid = f.facid
